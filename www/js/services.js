angular.module('starter.services', [])
  ...
  .service('Tab1Service', function ($http) {
    this.getList = function (c) {
        return $http.get(c.url + "?page=" + c.page + "&rows=" + settings.rows);
    }
    this.getClassify = function () {
      return [
        {
          name: '健康资讯', isInit: true, url: server.domain + '/info/list',
          page: 1, rows: 20, items: [], getList: this.getList
        },
        {
          name: '健康知识', isInit: true, url: server.domain + '/lore/list',
          page: 1, rows: 20, items: [], getList: this.getList
        },
        {
          name: '健康问答', isInit: true, url: server.domain + '/ask/list',
          page: 1, rows: 20, items: [], getList: this.getList
        }
        // ,{
